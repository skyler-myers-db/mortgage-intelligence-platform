...
| `DATABRICKS_ACCOUNT_CLIENT_ID` | Dedicated account-SCIM OAuth client used for exact identity/role inspection and bounded group-owner exclusion. | Must differ from every app-facing M2M and target App. A first install with a group-owned UC object requires account-admin authority because the bundle-created target App cannot be delegated in advance. After first install, downscope to Service Principal Manager on every forbidden normal, operator2, admin, verifier, agent-runtime, and now-existing target App so five-minute target credentials can be created and revoked. Deployment fails closed if credentials or proof are absent or inconclusive. |
