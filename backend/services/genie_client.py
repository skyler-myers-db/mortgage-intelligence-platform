...
# ---------------------------------------------------------------------------
# The resilience wrapper and the process-wide registry live in
# ``genie_client_registry`` (file-size split). They are re-exported here so
# every existing ``from backend.services.genie_client import ...`` call site
# and every ``monkeypatch.setattr("backend.services.genie_client", ...)``
# target keeps working. The import sits at the bottom, after the classes
# above are bound, because the registry module imports ``GenieClient`` from
# this module; either import order resolves.
# ---------------------------------------------------------------------------
from backend.services.genie_client_registry import (  # noqa: E402
    ResilientGenieClient,
    _load_space_id_from_file,  # noqa: F401 - compatibility re-export
    _reset_genie_client_for_tests,  # noqa: F401 - compatibility re-export
    _resolve_space_id,  # noqa: F401 - compatibility re-export
    get_genie_client,
    is_placeholder_space_id,  # noqa: F401 - compatibility re-export
)
