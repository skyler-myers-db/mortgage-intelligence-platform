...
| `genie_job_finished` (logger `mip-genie`), status `cancelled` | INFO | `status` cancelled, `duration_ms`, `job_id`, no `failure_kind` | The runner stopped a job whose cancel came before its governed record (at a stage boundary, or refused by the `recorded_at` commit point): no RUN_GENIE row, no action tokens, no session row. Not a failure. |
