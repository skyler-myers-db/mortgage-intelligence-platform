...
**Root cause:** `sql/transformations/gold_segment_population.sql` aggregated segment rows from `gold.borrower_360.segment_codes` via `LATERAL VIEW EXPLODE`. The `listed` and `permit` predicates are intentionally blocked-FALSE in `gold.borrower_360` (pending Cotality MLS + Building-Permits Delta Share arrival per `frontend/src/routes/segment-intelligence.tsx:207–216`), so no row in the source carried those codes. The CTAS therefore correctly returned 4 rows — the data was honest, but the segment registry contract was implicit instead of explicit.
...
**FE companion:** `frontend/src/components/mortgage/SegmentCard.tsx` now detects `count === 0` AND `code in {listed, permit}` and renders a "Pending source" variant — same `.seg-card` shell + `--seg-color` accent bar so the segment retains visual identity, but with "Awaiting Cotality MLS share" / "Awaiting Cotality Permits share" microcopy in the meta row, no hover lift, no click handler. CSS `seg-card--pending` block added in `frontend/src/design-system/components.css`. Honest UX: the user sees all 6 segments and the upstream data dependency is called out inline.
...
- **Borrower ID format drift (B-##### vs B-XXXXXXXXXXXXX).** Production IDs are 13-char alphanumeric (`B-102FL7THC6Q3L`); the spec docstring "B-#####" is outdated copy. Not a contract bug. Recommend updating `CLAUDE.md` Naming Rules section in a doc-only PR.
...
This shell does not have the `databricks` CLI, so the bundle deploy (`databricks bundle deploy -t dev` then `databricks bundle run refresh_silver -t dev`) must be run from a workstation with credentials. Once deployed, the Segment Intelligence page should render 6 cards with Listed for Sale and Permit Activity in the "Pending source" state until the Cotality MLS and Building-Permits Delta Shares land in Unity Catalog.
...
- When the Cotality MLS Delta Share lands, drop the `listed_for_sale = FALSE` block in `gold.borrower_360` and the SegmentCard pending state will auto-clear. Same for Permits.
