...
| `/` | Choropleth state click | Drill to county level | LIVE (superseded finding) | Current map counts and top segment values come from backend geography rollups. |
| `/` | Choropleth non-supported state click | Select state or render unavailable drill state | LIVE (superseded finding) | Current behavior depends on available source coverage and geometry, not a fixed demo-state list. |
| `/` | Choropleth county click | Select county; drill to ZIP when source rollups exist | LIVE (superseded finding) | Current county and ZIP levels are driven by backend rollups. |
| `/` | Choropleth ZIP click | Navigate to borrower detail only when a live borrower id is present | LIVE (superseded finding) | Current ZIP tiles use current backend rollup context rather than local fixture borrower ids. |
...
| `/` | Map hover tooltip | Show name / count / score / top segment | LIVE (superseded finding) | Data source is the backend geography rollup. |
...
- **Evidence drawer values now avoid frozen row-count/model-metric claims.** Drawer source metadata describes governed assets and signal semantics; borrower/count/freshness values must come from API payloads or evidence events, not local literal metrics.
- **Choropleth geography facts now use backend rollups.** The earlier local geography fixture finding is superseded. Current state/county/ZIP counts, average scores, and top segments are served from gold-backed geography rollup endpoints.
- **ZIP drill-down only navigates for a subset of displayed ZIPs.** Some ZIP tiles have `borrowerId` set while others are visually identical but select without navigating. Fix: either populate `borrowerId` for every displayed ZIP tile, or dim non-navigable ZIPs to signal unavailable state.
...
- Previous local geography fact tables were removed from the current implementation. Current state/county/ZIP values are derived from backend rollups and live source coverage.
