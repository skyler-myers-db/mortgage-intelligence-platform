 ...
 * with the current print annotated and the lender's refi screen drawn as a
 ...
 */
export function RateWindowSection() {
  const query = useWarmingUpRetry<RateWindowResponse>(
  ...
  return (
    <LoadState query={query} title={RATE_WINDOW_TITLE}>
      {(data) => <RateWindowPanel data={data} />}
    </LoadState>
  );
}

export function RateWindowPanel({ data }: { data: RateWindowResponse }) {
    ...
    <section className="surface analytics-section rate-window" data-testid="rate-window" aria-labelledby="rate-window-title">
      <div className="surface__hdr surface__hdr--split">
        <div>
          ...
          {model ? (
            ...
            <p className="analytics-panel-note rate-window__summary" data-testid="rate-window-summary">
              No weekly market rates have landed yet; the FRED seed populates this on the first refresh.
            </p>
          )}
        </div>
      ...
      <div className="surface__body">
        ...
        <p className="muted fs-12 rate-window__asof" data-testid="rate-window-asof">
          ...
          it stood that week. Fixed-rate first liens only.
        </p>
      </div>
    </section>
  );
...
function RateWindowCharts({ model }: { model: RateWindowModel }) {
  ...
  const label = `${RATE_WINDOW_TITLE}. ${model.spreadSentence} ${model.itmSentence}`;
  return (
    <div className="rate-window__panels" role="img" aria-label={label}>
      <div className="rate-window__panel rate-window__panel--rates" data-testid="rate-window-rates">
        ...
        <div className="rate-window__legend" aria-hidden="true">
          ...
          {model.threshold && (
            <span className="rate-window__legend-item"><span className="rate-window__swatch rate-window__swatch--threshold" /> Refi screen</span>
          )}
        </div>
      </div>
