...
describe('smsSegments (critic-02)', () => {
  it('counts nothing for empty text', () => {
    expect(smsSegments('')).toEqual({ encoding: 'GSM-7', units: 0, segments: 0, perSegment: 160 });
  });

  it('sends GSM-7 as one 160-septet message, then 153-septet parts', () => {
    expect(smsSegments(a(160))).toEqual({ encoding: 'GSM-7', units: 160, segments: 1, perSegment: 160 });
    expect(smsSegments(a(161))).toEqual({ encoding: 'GSM-7', units: 161, segments: 2, perSegment: 153 });
  ...
  it('keeps the whole GSM 03.38 basic table (except escape) at one septet', () => {
    ...
    expect(smsSegments(basic)).toEqual({ encoding: 'GSM-7', units: 127, segments: 1, perSegment: 160 });
  });

  it('charges the extension table two septets each and never splits an escape pair', () => {
    expect(smsSegments('^{}\\[~]|€\f')).toMatchObject({ encoding: 'GSM-7', units: 20, segments: 1 });
    expect(smsSegments('€'.repeat(80))).toMatchObject({ units: 160, segments: 1 });
    // 152 + 2 + 152 = 306 septets: two parts by arithmetic, three when the
    // euro's escape pair cannot straddle the first boundary.
    expect(smsSegments(`${a(152)}€${a(152)}`)).toEqual({ encoding: 'GSM-7', units: 306, segments: 3, perSegment: 153 });
  });

  it('switches the whole message to UCS-2 on one non-GSM character: 70 single, 67 per part', () => {
    const curly = `${a(69)}’`;
    expect(smsSegments(curly)).toEqual({ encoding: 'UCS-2', units: 70, segments: 1, perSegment: 70 });
    expect(smsSegments(`${curly}a`)).toEqual({ encoding: 'UCS-2', units: 71, segments: 2, perSegment: 67 });
  ...
  it('charges a surrogate pair two units and never splits it across parts', () => {
    expect(smsSegments('😀')).toEqual({ encoding: 'UCS-2', units: 2, segments: 1, perSegment: 70 });
    expect(smsSegments(`${a(66)}😀${a(66)}`)).toEqual({ encoding: 'UCS-2', units: 134, segments: 3, perSegment: 67 });
  });
  ...
  it('reads a governed single-segment SMS as GSM-7', () => {
    ...
    expect(smsSegments(body)).toEqual({ encoding: 'GSM-7', units: body.length, segments: 1, perSegment: 160 });
  });
});
