...
CREATE TABLE IF NOT EXISTS mip.gold.borrower_360 (
  ...
  county_fips_5             STRING             COMMENT 'NULL: the Cotality share carries one constant fips_county_code per state (2026-08-07 audit C2), so no honest county key exists. Sub-state geography truth is ZIP (gold.zip_rollup).',
  ...
  rate_spread_bps           INT       NOT NULL COMMENT 'fn_rate_spread(first_pos_rate, market_rate_fraction), gated on an ACTIVE lien and an in-bounds rate: 0 when the amortized UPB estimate is fully paid down (nothing to refinance) or when the bounded rate sits at the 1%/15% clamp (unknown true rate). Positive = above market = refi opportunity.',
  ...
  approval_status           STRING    NOT NULL COMMENT 'Approval state mirrored from Lakebase via gold.borrower_lifecycle_state at refresh time; "pending" when undecided. Lakebase remains authoritative between refreshes.',
  ...
  lien_is_portfolio_level   BOOLEAN   NOT NULL COMMENT 'TRUE when the estimated lien balance is a blanket/portfolio instrument amount attributed to a single CLIP (balance > 10x AVM and > $5M, or > $20M): real instrument, not property-level UPB. Excluded from headline dollar totals; display LTV falls back to modeled CLTV.',
  ...
  current_rate              DOUBLE    NOT NULL COMMENT 'PERCENT form (5.75, not 0.0575). 0.0 when no active lien remains (paid-off loans carry no current rate). Matches Pydantic current_rate and mock_data convention.',
  ltv                       INT       NOT NULL COMMENT 'Display LTV int from estimated current lien balance divided by AVM. Bounded to 0..500: above 5x the valuation the number is not a property-level loan-to-value, and 0 with ltv_basis_is_unreliable=TRUE means UNKNOWN, not free-and-clear. Underwater borrowers still show >100.',
  ltv_basis_is_unreliable   BOOLEAN   NOT NULL COMMENT 'TRUE when no trustworthy display-LTV basis exists: AVM below the $10k plausibility floor, or lien over 5x AVM (blanket/portfolio attribution), AND modeled CLTV missing or itself over 500%. The API withholds ltv when TRUE; avm_value and current_lien_balance stay populated.',
...
CREATE TABLE IF NOT EXISTS mip.gold.lockin_cohort (
  ...
  first_pos_loan_type   STRING             COMMENT 'CNV / FHA / VA / other; from silver.',
...
CREATE TABLE IF NOT EXISTS mip.gold.borrower_dossier (
  ...
  ltv                       INT       NOT NULL COMMENT 'Display LTV int, bounded to 0..500; underwater borrowers may exceed 100. 0 with ltv_basis_is_unreliable=TRUE means UNKNOWN, not free-and-clear.',
  ltv_basis_is_unreliable   BOOLEAN   NOT NULL COMMENT 'TRUE when no trustworthy display-LTV basis exists (mirrors borrower_360). The API withholds ltv when TRUE; avm_value and current_lien_balance stay populated.',
...
-- -----------------------------------------------------------------------------
-- 19. mip.gold.rate_sensitivity_book
--     Rate Lever support table (wow-stage-1): the gated note rate
--     (NOTE_RATE_GATE_SQL over gold.borrower_360 LEFT JOIN silver.lien_current)
--     of every borrower a par-rate scenario can move, one row per CLIP. The
--     endpoint's live contactable aggregate LEFT JOINs it so the App reads
--     gold only (mip.silver is ETL-only, docs/security/GRANTS.md section 5).
--     See sql/ddl/gold_rate_sensitivity_book.sql for column comments.
-- -----------------------------------------------------------------------------
