...
CREATE TABLE IF NOT EXISTS mip.gold.borrower_360 (
  ...
  rate_spread_bps           INT       NOT NULL COMMENT 'fn_rate_spread(first_pos_rate, market_rate_fraction), gated on an ACTIVE lien: 0 when the amortized UPB estimate is fully paid down (nothing to refinance). Positive = above market = refi opportunity.',
