...
--            after the first demo refresh (out-of-band, not DDL).
--
...
--            file. Full rebuild is the demo posture per data-contract §3.2
--            -- 5M rows in minutes on serverless; precomputed gold is cheap
--            to refresh, and the CTAS guarantees no stale columns from
...
CREATE TABLE IF NOT EXISTS mip.gold.borrower_360 (
  ...
  is_current_customer       BOOLEAN   NOT NULL COMMENT 'UPPER(first_pos_lender_current) LIKE "%SUMMIT%". Demo lender per CLAUDE.md. Production swaps to ref.lender_dictionary join.',
