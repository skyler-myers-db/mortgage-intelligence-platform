...
COMMENT ON COLUMN mip.gold.borrower_360.has_heloc_draw_ending IS 'S1.3 heloc_draw_to_payback segment flag: open equity-loan lien originated 102-126 months ago, i.e. the standard 120-month draw ends within 18 months or ended within the last 6.';
...
COMMENT ON COLUMN mip.gold.borrower_360.refi_propensity_heuristic IS 'S1.3: fn_refi_propensity_heuristic(rate_spread_bps, first_pos_age_months, equity_pct, current_lien_balance, listed_for_sale). TRANSPARENT deterministic points table 0..100 (published in the glossary). NOT the Cotality refi propensity model score.';
...
COMMENT ON COLUMN mip.gold.borrower_360.itm_on_related_property IS 'S1.3 itm_on_related_property segment flag: any Owner Link on this CLIP (S1.1 silver.property_owners, all slots) also holds a DIFFERENT clip that is in the money under this refresh''s thresholds.';
