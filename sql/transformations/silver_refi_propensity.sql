-- =============================================================================
-- silver_refi_propensity.sql (transformation)
-- -----------------------------------------------------------------------------
-- Purpose:   Idempotent MERGE from Cotality refinance propensity score into
--            mip.silver.refi_propensity.
-- =============================================================================

MERGE INTO mip.silver.refi_propensity AS t
USING (
  SELECT
    clip,
...
WHEN MATCHED THEN UPDATE SET
  ...
  refi_propensity_score = s.refi_propensity_score,
  refi_propensity_run_date = s.refi_propensity_run_date,
...
WHEN NOT MATCHED THEN INSERT (
  clip, situs_state, situs_zip_code, refi_propensity_score,
  refi_propensity_run_date, source_updated_at, ingest_ts,
  _meta_source_file_name, _meta_batch_id
) VALUES (
  s.clip, s.situs_state, s.situs_zip_code, s.refi_propensity_score,
  s.refi_propensity_run_date, s.source_updated_at, s.ingest_ts,
