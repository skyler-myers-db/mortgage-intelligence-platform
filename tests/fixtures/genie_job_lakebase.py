...
class FakeJobLakebase:
    ...
    def __init__(self, *, jobs_table: bool = True, now: datetime | None = None) -> None:
    ...
    def _view(self, row: dict[str, Any]) -> dict[str, Any]:
        view = {key: row[key] for key in row if key not in {"actor_email", "conversation_id", "message_id", "finished_at"}}
    ...
    def fetchone(self, sql: str, params: dict[str, Any] | None = None) -> dict[str, Any] | None:
        ...
        with self._lock:
            if sql is jobs._PROBE_SQL:
                return {"present": self.jobs_table}
            if sql in _JOB_SQL:
                self._require_table()
    ...
    def _job_fetchone(self, sql: str, params: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if sql is jobs._CLAIM_SQL:
            if row["status"] != "queued" or not owned:
                return None
