...
def test_native_analytics_routes_return_typed_app_payloads() -> None:
    ...
    assert set(signals.json()) == {"evidence_daily", "evidence_by_signal"}
...
class _AnalyticsSqlClient:
    ...
    def execute(self, statement: str, _parameters: object | None = None) -> list[dict[str, object]]:
        ...
        if "funnel_snapshot_daily" in statement and "LIMIT 1" in statement:
            return [{
                ...
                "approved_borrowers": 1,
                "actioned_borrowers": 1,
            }]
        if "borrower_lifecycle_state" in statement and "approved_borrowers" in statement:
            return [{"approved_borrowers": 2, "actioned_borrowers": 1}]
        ...
        if "SIZE(segment_codes)" in statement:
            return [{
                "borrower_id": "B-48291",
        ...
        if "ROW_NUMBER() OVER (ORDER BY opportunity_score DESC, clip)" in statement:
        ...
        if "sp.approval_rate" in statement:
        ...
        if "WHERE state <> '_ALL'" in statement and not statement.startswith("WITH ranked"):
        ...
        if "TO_DATE(`timestamp`)" in statement:
            return [{"event_date": "2026-05-18", "signal_type": "rate_spread", "event_count": 3}]
        if "AVG(confidence)" in statement:
            return [{"signal_type": "rate_spread", "source_product": "Voluntary Lien", "event_count": 3, "mean_confidence": 0.9}]
...
def test_analytics_repository_uses_governed_gold_and_semantic_sql() -> None:
    ...
    scatter_sql = next(sql for sql in client.statements if "SIZE(segment_codes)" in sql)
    assert "borrower_id AS borrower_id" in scatter_sql
    assert "display_name AS display_name" in scatter_sql
    assert "segment_codes[0]" in scatter_sql
    ...
    top_borrower_sql = next(sql for sql in client.statements if "ROW_NUMBER() OVER (ORDER BY opportunity_score DESC, clip)" in sql)
