...
def test_provisions_required_values_and_generated_previous_without_printing(
    ...
    monkeypatch.setenv("MIP_COTALITY_ID_MASK_SECRET", "mask-value")
    monkeypatch.setenv("MIP_GENIE_ACTION_SECRET_CURRENT", "current-value")
    ...
    monkeypatch.setattr(subject.secrets, "token_urlsafe", lambda _size: "generated-previous")
    ...
    assert secrets_api.puts == [
        (subject.DEFAULT_SCOPE, "cotality-id-mask-v1", "mask-value"),
        (subject.DEFAULT_SCOPE, "genie-action-current", "current-value"),
        (subject.DEFAULT_SCOPE, "genie-action-previous", "generated-previous"),
    ]
